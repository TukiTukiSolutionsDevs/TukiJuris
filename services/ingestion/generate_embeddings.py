...
# Fernet decryption — mirrors LLMKeyEncryption versioned logic.
# New rows use BYOK_ENCRYPTION_KEY with "v1:" prefix.
# Legacy rows use JWT_SECRET-derived key (no prefix).
_JWT_SECRET = os.getenv("JWT_SECRET", "dev-only-secret-change-in-production")
_BYOK_ENCRYPTION_KEY = os.getenv("BYOK_ENCRYPTION_KEY", "")
_V1_PREFIX = "v1:"


def _decrypt_fernet(encrypted: str) -> str:
    """Decrypt a versioned Fernet-encrypted API key.

    Supports:
      - "v1:<ciphertext>" — uses BYOK_ENCRYPTION_KEY (new format)
      - "<ciphertext>"    — uses JWT_SECRET-derived key (legacy format)
    """
    import base64
    import hashlib
