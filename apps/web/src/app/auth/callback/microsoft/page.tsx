...
function MicrosoftCallbackInner() {
  ...
  useEffect(() => {
    ...
    if (!code) {
      toast.error("No se recibio el codigo de autorizacion de Microsoft.");
    ...
    const exchange = async () => {
      try {
        const res = await fetch("/api/auth/oauth/microsoft/callback", {
          method: "POST",
          credentials: "include",
        ...
        if (!res.ok) {
          ...
          toast.error(data.detail ?? "Error al autenticar con Microsoft.");
          router.push("/login?error=oauth_failed");
          return;
        }
