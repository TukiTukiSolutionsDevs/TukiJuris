/**
 * Tests for returnto threading in the Login page SSO buttons.
 *
 * Covers:
...
vi.mock("@/lib/auth/jwt", () => ({
  decodeAccessClaims: vi.fn().mockReturnValue(null),
}));

vi.mock("@/lib/auth/redirects", () => ({
  resolvePostLoginDestination: vi.fn().mockReturnValue("/chat"),
}));
...
const OAUTH_REDIRECT = "https://accounts.google.com/o/oauth2/auth?state=xyz";
...
beforeEach(() => {
  // Default: currently on the login page itself
...
describe("LoginPage — Google SSO returnto", () => {
  it("omits returnto when already on /auth/login", async () => {
  ...
  it("omits returnto when on /auth/register", async () => {
...
describe("LoginPage — Microsoft SSO returnto", () => {
  it("omits returnto when already on /auth/login", async () => {
