...
export default function OrganizacionPage() {
  ...
  const loadOrg = useCallback(async () => {
    ...
    try {
      ...
      if (orgs.length > 0) {
        ...
        // Load usage
        const usageRes = await authFetch(`/api/billing/${first.id}/usage`);
        if (usageRes.ok) {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await authFetch(`/api/organizations/${org.id}/invite`, {
        method: "POST",
  ...
  const handleRemoveMember = async (userId: string) => {
    ...
    setError("");
    try {
      const res = await authFetch(`/api/organizations/${org.id}/members/${userId}`, {
        method: "DELETE",
      });
