    ...
    async (isRefresh = false) => {
      ...
      try {
        const [overviewRes, areasRes, modelsRes, queriesRes, costsRes, topQueriesRes] =
          await Promise.all([
            fetch(`${API_URL}/api/analytics/${orgId}/overview?days=${days}`, { headers }),
            fetch(`${API_URL}/api/analytics/${orgId}/areas?days=${days}`, { headers }),
            fetch(`${API_URL}/api/analytics/${orgId}/models?days=${days}`, { headers }),
            fetch(`${API_URL}/api/analytics/${orgId}/queries?per_page=20`, { headers }),
            fetch(`${API_URL}/api/analytics/${orgId}/costs?days=${days}`, { headers }),
            fetch(`${API_URL}/api/analytics/${orgId}/top-queries?days=${days}&limit=15`, { headers }),
          ]);
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/analytics/${orgId}/export?days=${days}`,
        { headers }
      );
