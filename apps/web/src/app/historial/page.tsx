...
export default function HistorialPage() {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      const res = await authFetch(
        `/api/conversations/?status=${statusParam}`
      );
    ...
    try {
      await Promise.all(
        Array.from(selected).map((id) => {
          ...
          return authFetch(`/api/conversations/${id}/${endpoint}`, { method });
        })
      );
      fetchConversations();
    } finally {
  ...
  const handleAssignTag = async (convId: string, tagId: string) => {
    await authFetch(`/api/conversations/${convId}/tags/${tagId}`, {
      method: "POST",
    });
    setTagDropdownConv(null);
    fetchConversations();
  ...
  const handleDeleteFolder = async (folderId: string) => {
    ...
    try {
      await authFetch(`/api/folders/${folderId}`, {
        method: "DELETE",
      });
  ...
  const handleMoveToFolder = async (convId: string, folderId: string | null) => {
    if (folderId) {
      await authFetch(`/api/conversations/${convId}/folder/${folderId}`, {
        method: "PUT",
      });
    } else {
      await authFetch(`/api/conversations/${convId}/folder`, {
        method: "DELETE",
      });
    }
    setFolderMenuConv(null);
