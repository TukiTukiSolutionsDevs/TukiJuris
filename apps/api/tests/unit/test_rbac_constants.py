...
class TestPermissionConstants:
    def test_total_count(self):
        assert len(ALL_PERMS) == 27

    def test_all_unique(self):
        assert len(set(ALL_PERMS)) == 27, "Duplicate permission detected"
    ...
    def test_impersonate_removed(self):
        assert "users:impersonate" not in ALL_PERMS, "impersonate permission must be absent from ALL_PERMISSIONS"


class TestPrivilegedRoleNames:
    def test_is_frozenset(self):
        assert isinstance(PRIVILEGED_ROLE_NAMES, frozenset)

    def test_contains_four_privileged_roles(self):
        assert len(PRIVILEGED_ROLE_NAMES) == 4

    def test_matches_system_role_values(self):
        expected = {
        ...
        assert PRIVILEGED_ROLE_NAMES == expected

    def test_viewer_not_privileged(self):
        assert SystemRole.VIEWER.value not in PRIVILEGED_ROLE_NAMES
