...
class TestRevokeAllReason:
    """revoke_all() logs the passed reason string."""

    async def test_revoke_all_default_reason_is_logout_all(self, caplog):
        """Calling revoke_all without reason logs reason=logout_all."""
        uid = uuid.uuid4()
        mock_repo = AsyncMock()
        mock_repo.revoke_all = AsyncMock(return_value=2)
        svc = _make_svc(mock_repo=mock_repo)
        ...
        reasons = [e.get("reason") for e in extras if "reason" in e]
        assert "logout_all" in reasons, f"Expected reason='logout_all' in log, got: {extras}"

    async def test_revoke_all_accepts_custom_reason(self, caplog):
        """Calling revoke_all(reason='admin_role_change') logs that value."""
        uid = uuid.uuid4()
        mock_repo = AsyncMock()
        mock_repo.revoke_all = AsyncMock(return_value=1)
        svc = _make_svc(mock_repo=mock_repo)

        with caplog.at_level(logging.INFO, logger="app.services.refresh_token_service"):
            await svc.revoke_all(uid, reason="admin_role_change")

        extras = [r.__dict__ for r in caplog.records]
        reasons = [e.get("reason") for e in extras if "reason" in e]
        assert "admin_role_change" in reasons, (
            f"Expected reason='admin_role_change' in log, got: {extras}"
        )
