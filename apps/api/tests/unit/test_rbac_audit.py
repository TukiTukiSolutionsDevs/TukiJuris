...
class TestAssignRoleAudit:
    """assign_role — audit behaviour.

    Note: assign_role delegates to _sync_is_admin (dual-write from
    fix-role-model-consistency). These tests stub _sync_is_admin to keep the
    scope on assign_role's own audit emission; sync_is_admin has its own
    dedicated tests.
    """
    ...
    async def test_assign_role_logs_audit_entry(self):
        ...
        svc._sync_is_admin = AsyncMock()  # isolate assign_role's own audit emission
        user_id = uuid.uuid4()
        role_id = uuid.uuid4()
    ...
    async def test_assign_role_no_audit_when_audit_is_none(self):
        """Backward compat: audit=None → no audit emission, no role lookup."""
        from app.rbac.service import RBACService
        ...
        svc._sync_is_admin = AsyncMock()
        # Must not raise; with audit=None, the role-name lookup is skipped so
        # db.execute stays un-called. (sync_is_admin is stubbed so its own
        # queries don't count.)
        await svc.assign_role(uuid.uuid4(), uuid.uuid4(), None)
...
class TestRevokeRoleAudit:
    """revoke_role — audit behaviour.

    Note: revoke_role delegates to _sync_is_admin (dual-write). These tests
    stub _sync_is_admin to keep the scope on revoke_role's own audit emission.
    """
    ...
    async def test_revoke_role_no_audit_when_audit_is_none(self):
        ...
        # Only the DELETE is executed — no role lookup, and sync_is_admin stub
        # doesn't touch db. Total: 1 execute.
