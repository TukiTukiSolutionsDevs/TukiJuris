...
# ---------------------------------------------------------------------------
# Autouse fixture — BETA_MODE must be explicit in production
# ---------------------------------------------------------------------------
#
# validate_production_config reads BETA_MODE directly from os.environ (see
# startup_validation.py §4). Most tests here assert the "no-raise" happy path,
# so we set BETA_MODE=true globally and let individual tests override when
# they need to exercise the "missing BETA_MODE" error path.


@pytest.fixture(autouse=True)
def _set_beta_mode_env(monkeypatch: pytest.MonkeyPatch) -> None:
