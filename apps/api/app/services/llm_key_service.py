...
Keys are encrypted at rest via LLMKeyEncryption (versioned Fernet, v1: prefix).
Legacy rows (no prefix, JWT_SECRET-derived) are lazily migrated on first USE.
"""
...
def encrypt_key(api_key: str) -> str:
    """Encrypt an API key. Returns versioned ciphertext ('v1:<ct>')."""
    return llm_key_crypto.encrypt(api_key)


def decrypt_key(encrypted: str) -> str:
    """Decrypt a stored API key. Handles v1 prefix and legacy (no-prefix) format."""
    plaintext, _needs_reencrypt = llm_key_crypto.decrypt(encrypted)
    return plaintext
...
async def get_user_key_for_provider(
    ...
    Legacy rows (pre-v1 format) are lazily re-encrypted with the current key on read.
    Uses SELECT FOR UPDATE to prevent concurrent re-encryption of the same row.
    """
    ...
    if not key_row:
        return None
    try:
        plaintext, needs_reencrypt = llm_key_crypto.decrypt(key_row.api_key_encrypted)
    except Exception:
        logger.warning(
        ...
        return None
    if needs_reencrypt:
        # Lazy migration: upgrade legacy row to v1 format in place.
        key_row.api_key_encrypted = llm_key_crypto.encrypt(plaintext)
        key_row.updated_at = datetime.now(UTC)
        await db.flush()
        logger.info(
            "byok.lazy_migration: user=%s provider=%s — key migrated to v1 format",
            user_id,
            provider,
        )
    return plaintext
