...
class RefreshTokenService:
    ...
    async def rotate(self, refresh_jwt: str, device_info: dict) -> TokenPair:
        ...
        # Fetch user to read is_admin — single PK lookup, negligible cost
        user_row = await self._session.get(UserModel, row.user_id)
        is_admin = bool(user_row.is_admin) if user_row else False
        access_jwt = create_access_token(data={"sub": str(row.user_id), "is_admin": is_admin})
    ...
    async def revoke_all(self, user_id: uuid.UUID, reason: str = "logout_all") -> int:
        ...
        Args:
            ...
            reason: Free-form label for observability.
                Known values: ``logout_all``, ``admin_role_change``, ``reuse_detected``.
        ...
        logger.info(
            "refresh.revoked",
            extra={"user_id": str(user_id), "reason": reason, "count": count},
        )
