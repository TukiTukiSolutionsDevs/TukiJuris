...
# Coarse IP-based flood protection at the middleware layer.
# Per-plan per-bucket limits are enforced at the route level via RateLimitGuard.
_IP_REQUESTS_PER_MINUTE = 120
_WINDOW_SECONDS = 60
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    Keys every request on client IP address — acts as a coarse flood shield.
    Fine-grained per-plan limits (READ/WRITE buckets) are enforced by
    RateLimitGuard at the route level.
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        # Skip IP bucket for authenticated requests on non-auth paths.
        # Authenticated = tk_session cookie present OR Authorization header present.
        # Auth-adjacent paths (login, register, oauth) always retain IP counting
        # as an anonymous abuse guard regardless of auth state.
        is_auth_adjacent = any(
            request.url.path.startswith(p) for p in _AUTH_ADJACENT_PREFIXES
        )
        is_authenticated = (
            "tk_session" in request.cookies
            or "authorization" in request.headers
        )
        if is_authenticated and not is_auth_adjacent:
            start = time.time()
            response = await call_next(request)
        ...
        key = f"ip:{client_ip}"
        max_requests = _IP_REQUESTS_PER_MINUTE
